...
void Scene::Scene_Init(LPDIRECT3DDEVICE9 d3ddev)
{
	...
	//D3DXCreateFontIndirect(d3ddev, &desc, &fps);
	desc.Height = 40;
	desc.Width = 20;
	desc.Weight = 400;
...
void Scene::Scene_Run()
{
	...
	if (end_render - begin_render > 0 && end_render - begin_render < 1000)
		ifps = 1000 / (end_render - begin_render);
	else 
		ifps = 1000;
...
void Scene::RenderSceneInfoPlayer()
{
	...
	//drawFont(fps, 600, 10, 35, 25, ifps);
}
